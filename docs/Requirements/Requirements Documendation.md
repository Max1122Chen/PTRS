...
| FR-004-5-2 | 室外地图入口与室内视图切换 | 高 | 路线规划页室外 **ECharts** 图上，POI 以节点展示；`indoorAvailable=true` 的 POI 须有**可区分样式**（如图标、描边或角标）。用户**点击**该节点后进入**室内视图**（非新浏览器页亦可，但须明确模式切换）；提供「返回室外地图」入口。未具备室内图的 POI 点击行为与现网一致（选点/详情），不进入室内视图。 | 室外图可识别并点击进入室内模式；可返回室外图 |
...
### 4.2.1 室内导航交互流程（FR-004-5）
```mermaid
flowchart TD
    A[室外路线规划页 ECharts] --> B{点击 POI}
...
| 维度 | 数量 | 说明 |
|------|------|------|
| 功能需求（FR）合计 | **48** 条（含子项） | 见 §9.2 全量清单 |
| **已实现** | **38** | 主路径可演示、验收口径基本满足 |
| **部分实现** | **8** | 核心可用，与规格有明确差距（见 §9.4） |
...
| 编号 | 名称 | 状态 | 实现要点 / 差距 |
|------|------|------|-----------------|
| FR-004 | 最优参观线路规划 | **部分实现** | 已实现距离/时间/多点 TSP；**未**单独提供「景点覆盖最多」策略 |
...
| FR-004-5 | 室内导航（按 POI） | **已实现** | 室外 ECharts 点选进入室内视图，至少 1 个演示 POI |
...
**室内演示路径**：dev-seed `502`（areaId **201**）；OSM 包沙河-南丰路 `900020599`（areaId **248**）。

#### 9.2.3 旅游中 — 场所与美食

| 编号 | 名称 | 状态 | 实现要点 / 差距 |
|------|------|------|-----------------|
| FR-005 | 景点介绍 | **已实现** | 景区详情页、路线图 POI tooltip（名称/类型/位置/经纬度） |
| FR-006 | 场所查询 | **已实现** | `/api/facility/search`，按景区检索 |
| FR-006-1 | 附近设施查找 | **已实现** | 图最短路可达距离、200/500/1000 m 可调 |
| FR-006-2 | 类别过滤 | **已实现** | 前端类型下拉 |
| FR-006-3 | 类别名称查询 | **已实现** | 内存模糊匹配 + 可达距离排序 |
| FR-013 | 美食推荐 | **部分实现** | 1000 m 半径、TopK、加权排序已实现；**未**接入用户饮食偏好（辣度/口味） |
| FR-013-1 | 美食排序 | **已实现** | 堆/TopK 选前 10，权重可调（默认 0.3/0.5/0.2） |
| FR-013-2 | 菜系过滤 | **已实现** | 菜系条件过滤 |
| FR-013-3 | 美食模糊查询 | **部分实现** | NGram 倒排 + 部分匹配；**无**独立同义词表 |
| FR-013-4 | 查询结果排序 | **已实现** | 热度/评价/距离加权 |
...
| **室内导航** | 答辩演示脚本 | 路线页选 areaId **248** 或 **201** → 点击 `indoorAvailable` POI → 室内规划 |
