...
| 数据类型 | 规模要求 | 本期达成策略（S2 拍板 2026-06-08） |
|---------|----------|-------------------------------------|
| 景区和校园数量 | 至少200个（最低要求），系统应支持1000个以上的扩展 | **alias 复用**：`scenic_areas` 记录 ≥200，多个 areaId 可指向同一 canonical OSM 地图包，无需 200 份独立抓取 |
| POI数量 | 每个景区和校园不少于20个 | 以 **canonical 包** 内 POI 为准；alias 景区共享同一 POI 集 |
| 服务设施种类 | 不少于10种 | **`Facility.type` 去重 ≥10**；OSM 映射 + 种子补齐缺失类型 |
| 服务设施数量 | 不少于50个 | OSM `facilities.append` + 必要时 seed 补充 |
| 道路图边数 | 不少于200条 | OSM 路网通常已满足 |
| 系统用户数 | 不少于10人（最低要求），系统应支持10000个以上的用户 | dev-seed 用户 + 注册能力 |
| 具备室内图的 POI 数量 | 课设演示**至少 1 个** POI `indoor_available=true`；建议种子阶段目标 **≥3 个** | OSM 采集 + **R8 人工校正** 后达标 |

> 规模统计与 alias 规则详见 [S2 执行计划 §5–§6](../Tech/S2%20Data%20Governance%20Execution%20Plan.md)。
...
> **S2 v2 策略**：景区 ≥200 用 **alias 复用**；设施种类 ≥10 用 **type 枚举 + seed 补齐**。详见 [S2 执行计划](../Tech/S2%20Data%20Governance%20Execution%20Plan.md)。

| 指标 | 规格要求 | 当前合并规模 | 状态 | S2 达成路径 |
|------|----------|--------------|------|-------------|
| 景区/校园数 | ≥200（可扩展 1000+） | **25** | **未达** | R11b：`scenic-area-aliases.json` 扩至 ≥200 |
| 每景区 POI | ≥20 | 4 个景区 ≥20 POI | **部分满足** | 多 canonical OSM 包（R7） |
| 服务设施种类 | ≥10 种 | **6** 种 | **未达** | R11a：枚举 ≥10 + seed/OSM |
| 服务设施数量 | ≥50 | **16** | **未达** | OSM + seed |
| 道路边数 | ≥200 | **3520** | **已满足** | — |
| 用户数 | ≥10 | **6**（种子） | **未达** | 种子或注册 |
| 室内图 POI | 演示 ≥1 | **≥2** | **已满足** | R8 校正后复测 |

**答辩建议**：说明规格规模为「系统设计能力上限」；本期以 **少量高质量 OSM canonical 包 + alias 扩表 + 枚举/种子补齐** 满足 §5.3 计数口径，算法与交互以真实样本演示为主。
...
**v1.7.2 变更摘要**：S2 v2 拍板 — JSON-only 演示；景区 ≥200 采用 alias 复用 canonical 数据；设施 type ≥10 采用枚举+seed；衍生 seed 规则（美食绑餐饮 POI）；多包 OSM 协作流程见 [S2 执行计划](../Tech/S2%20Data%20Governance%20Execution%20Plan.md)。
