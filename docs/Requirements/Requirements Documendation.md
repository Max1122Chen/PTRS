...
### 9.2 尚未实现（当前优先：FR-009-5 实机验证）

| 需求编号 | 名称 | 说明 |
|----------|------|------|
| **FR-009-5** | 旅游动画生成（云端 + 落盘） | 主链路已在代码库落地（见 HANDOFF 2026-05-10）；需在目标环境完成密钥配置与实机出片验收。 |
...
| **FR-004-5**（含 5-1～5-4） | **室内导航**：`dev-seed/indoor/502.json`（图书馆 POI 502）、`IndoorController` 四接口、路线页室外/室内 ECharts 切换；完整度门禁与 Dijkstra 见 `IndoorSeedCompleteness` / `IndoorPathPlanner`（HANDOFF 2026-05-20）。 |
